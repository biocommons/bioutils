"""Compute sequence digests/IDs (VMC seqhash, seguid, md5/sha1/sha512) with optional normalization."""
...
def seq_seqhash(seq: str, normalize: bool = True) -> str:
...
def seq_seguid(seq: str, normalize: bool = True) -> str:
    ...
    return base64.b64encode(hashlib.sha1(bseq).digest()).decode("ascii").rstrip("=")  # noqa: S324


def seq_md5(seq: str, normalize: bool = True) -> str:
    ...
    seq = normalize_sequence(seq) if normalize else seq
    bseq = seq.encode("ascii")
    return hashlib.md5(bseq).hexdigest()  # noqa: S324


def seq_sha1(seq: str, normalize: bool = True) -> str:
    ...
    seq = normalize_sequence(seq) if normalize else seq
    bseq = seq.encode("ascii")
    return hashlib.sha1(bseq).hexdigest()  # noqa: S324


def seq_sha512(seq: str, normalize: bool = True) -> str:
...
def seq_vmc_id(seq: str, normalize: bool = True) -> str:
...
def seq_vmc_identifier(seq: str, normalize: bool = True) -> str:
    ...
    Examples:
        >>> seq_vmc_identifier("") == {
        ...     "namespace": "VMC",
        ...     "accession": "GS_z4PhNX7vuL3xVChQ1m2AB9Yg5AULVxXc",
        ... }
        True

        >>> seq_vmc_identifier("ACGT") == {
        ...     "namespace": "VMC",
        ...     "accession": "GS_aKF498dAxcJAqme6QYQ7EZ07-fiw8Kw2",
        ... }
        True
        ...
        >>> seq_vmc_identifier("acgt", normalize=False) == {
        ...     "namespace": "VMC",
        ...     "accession": "GS_eFwawHHdibaZBDcs9kW3gm31h1NNJcQe",
        ... }
        True
